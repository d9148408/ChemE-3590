...
- **$\alpha = 0$**：無正則化，等同於 OLS
- **$\alpha$ 很小**：輕微懲罰，接近 OLS
- **$\alpha$ 適中**：平衡擬合誤差與係數大小
- **$\alpha$ 很大**：強烈懲罰，係數接近 0，模型欠擬合

### 3.2 選擇 $\alpha$ 的方法

#### 方法 1: 交叉驗證

使用 `RidgeCV` 自動搜索最佳 $\alpha$ ：
...
- **化工過程建模**：通常從 $\alpha = 1$ 開始嘗試
- **高維數據**：可能需要較大的 $\alpha$ (10-100)
- **強多重共線性**：建議使用較大的 $\alpha$
- **始終使用交叉驗證**：數據驅動地選擇最佳 $\alpha$
...
- **挑戰**：Temperature 與 Pressure 存在相關性（相關強度設定 = 0.6，實際 Pearson 相關係數 ≈ 0.47），模擬實際化工過程中的多重共線性問題
...
- 樣本數：500 筆
- 特徵數：4 個
- 多重共線性設計：Temperature 與 Pressure 的相關強度如數据生成參數 0.6，實際測量得 Pearson r = 0.473
...
方差膨脹因子 (VIF):
         Feature       VIF
   Temperature_C  1.292532
    Pressure_bar  1.288541
    Catalyst_wt%  1.003663
        Time_min  1.000249
...
**分析結論**：
- Temperature_C 與 Pressure_bar 的 VIF 約為 1.29，顯示存在輕微的多重共線性
- 雖然 VIF < 5（無嚴重共線性），但仍適合使用 Ridge 回歸以穩定係數估計
...
- 最佳 $\alpha = 1.1514$，接近 1.0，表示需要適度的正則化強度
- 使用 5-Fold 交叉驗證自動選擇最佳參數

![Regularization Path](outputs/P3_Unit10_Ridge_Regression/figs/regularization_path.png)
*圖 6A.2：正則化路徑（係數隨 $\alpha$ 變化趨勢）*

**正則化路徑解讀**：
- 紅色虛線標示最佳 $\alpha = 1.1514$ 的位置
- 當 $\alpha$ 增大時，所有係數都向 0 收縮
- Temperature_C 的係數最大，受正則化影響最顯著
- 即使 $\alpha$ 很大，係數也不會完全變為 0（Ridge 的特性）
...
**結果解讀**：
1. **Condition 1（低強度）**：溫度 90°C，壓力 2.0 bar → 產率 79.20%
2. **Condition 2（中強度）**：溫度 100°C，壓力 3.5 bar → 產率 93.99%
3. **Condition 3（高強度）**：溫度 110°C，壓力 4.5 bar → 產率 105.89%
...
【Ridge Regression 優勢】
✓ 有效處理多重共線性問題 (Temperature 與 Pressure 相關係數 = 0.473)
✓ 透過係數縮減提升模型穩定性與泛化能力
...
2. **正則化參數選擇**： $\alpha \approx 1$ 提供了最佳的偏差-方差權衡
...
4. **考慮業務需求**：
   - 需要更穩定的模型：選擇較大的 $\alpha$
   - 需要更高的擬合精度：選擇較小的 $\alpha$
...
3. **正則化強度 $\alpha$**：控制偏差-方差權衡，需通過交叉驗證選擇
